...
export function RoadmapDashboard() {
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      try {
        ...
        if (!raw) {
          setState({ completedTaskIds: [...confirmedCompletedTaskIds], version: SCHEMA_VERSION });
          return;
        }
        const saved = JSON.parse(raw);
        const existingTaskIds = Array.isArray(saved.completedTaskIds) ? saved.completedTaskIds : [];
        const savedRevenue = typeof saved.actualRevenue === "number" ? saved.actualRevenue : undefined;
        const savedVersion = typeof saved.version === "number" ? saved.version : 1;

        if (savedVersion < SCHEMA_VERSION) {
          const mergedTaskIds = Array.from(new Set([...existingTaskIds, ...confirmedCompletedTaskIds]));
          setState({ completedTaskIds: mergedTaskIds, actualRevenue: savedRevenue, version: SCHEMA_VERSION });
          if (savedRevenue) setRevenueInput(String(savedRevenue));
        } else {
          setState({ completedTaskIds: existingTaskIds, actualRevenue: savedRevenue, version: savedVersion });
          if (savedRevenue) setRevenueInput(String(savedRevenue));
        }
      } catch {
        setState({ completedTaskIds: [...confirmedCompletedTaskIds], version: SCHEMA_VERSION });
  ...
  const allMilestones = useMemo(() => [...roadmapMilestones, checkpointMilestone], []);
  ...
  const activeIsDone = active.tasks.length > 0 && active.tasks.every((task) => completed.has(task.id));
  const activeStatus = date ? (activeIsDone ? "100% Selesai" : period(active, date) === "current" ? "Sedang berjalan" : period(active, date) === "upcoming" ? "Berikutnya" : "Lewat deadline") : "Memuat tanggal lokal";
  const daysLeft = date ? Math.max(0, Math.ceil((dateAtStart("2026-12-01").getTime() - dateAtStart(date).getTime()) / 86_400_000)) : null;
  const focusMilestone = (active.tasks.some((t) => !completed.has(t.id)) ? active : roadmapMilestones.find((item) => item.tasks.some((t) => !completed.has(t.id)))) ?? active;
  const priorityTasks = focusMilestone.tasks.filter((task) => !completed.has(task.id)).slice(0, 3);
  const isFocusM2 = focusMilestone.id === "m2" && activeIsDone;
  const toggle = (id: string) => setState((current) => ({ ...current, version: SCHEMA_VERSION, completedTaskIds: current.completedTaskIds.includes(id) ? current.completedTaskIds.filter((item) => item !== id) : [...current.completedTaskIds, id] }));
  const updateRevenue = (raw: string) => { const digits = raw.replace(/\D/g, ""); const number = Number(digits); setRevenueInput(digits); setState((current) => ({ ...current, version: SCHEMA_VERSION, actualRevenue: Number.isFinite(number) && number > 0 ? number : undefined })); };
  const reset = () => { if (window.confirm("Reset semua checklist dan omzet aktual yang tersimpan di browser ini?")) { setState({ completedTaskIds: [], version: SCHEMA_VERSION }); setRevenueInput(""); } };
  const revenueProgress = Math.min(100, Math.round(((state.actualRevenue || 0) / targetRevenue) * 100));
  return <><a className="skip-link" href="#roadmap-main">Lewati ke roadmap</a><header className="roadmap-header"><Link href="/" className="roadmap-brand"><strong>Pembelajar Belajar <i>/</i> Konten</strong><span>Internal Roadmap</span></Link><Link href="/" className="roadmap-back">← Kembali ke Landing Page</Link></header><main id="roadmap-main">
    ...
    <section className="roadmap-wrap roadmap-priority"><div><p className="roadmap-eyebrow">PRIORITAS SAAT INI</p><h2>Fokus utama sekarang: <em>{isFocusM2 ? "lengkapi sales kit." : `${focusMilestone.title.toLowerCase()}.`}</em></h2><p>{isFocusM2 ? "Landing page dan proof utama sudah siap. Fokus berikutnya adalah menyiapkan quotation, client brief, onboarding, WhatsApp outreach, follow-up, invoice, dan dokumen scope agar AI Content Factory siap menerima pelanggan pertama." : priorityTasks.length ? "Tiga langkah berikutnya untuk menjaga fokus harian." : "Semua milestone pada roadmap ini sudah selesai."}</p></div>{priorityTasks.length > 0 && <ol>{priorityTasks.map((task) => <li key={task.id}><button type="button" onClick={() => toggle(task.id)} aria-label={`Tandai selesai: ${task.label}`}>○</button>{task.label}</li>)}</ol>}</section>
